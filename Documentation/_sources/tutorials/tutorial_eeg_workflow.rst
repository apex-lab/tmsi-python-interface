...
labeling is loaded to the device (line 70). Please make sure to load the device configuration after opening a connection to the device.
...
:ref:`Plotters Tutorial <TMSi Plotter tutorial>`). In this case, the ImpedancePlotterHelper is initialized in line 75. Three parameters can be specified for the impedance plotter. The 
...
(line 88). Line 91 - 97 ensures that the specified file format is initialized. Here, it is also possible to specify the 
...
Please note, if neither .poly5 nor .xdf is given as input, the data will be saved as .poly5 (lines 95 - 97).
Moreover, it is required for the *file_writer* to get a handle to the device specified in the *discoveryList*, which is done in line 100.
The default file name is *example_EEG_workflow*. If the user wishes to change the file name, lines 91-97 can be edited. 
...
GUI is initialized based on the correct Helper and the GUI's event loop is started by line 107.
