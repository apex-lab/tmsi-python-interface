...
/**
 * @file ${emg_envelope_plotter.py} 
 * @brief Plotter object that displays the envelope of the signal after 
 * filtering and rectifying real-time data, designed for EMG applications
 *
 */
...
class EMGEnvelopeViewer():
    ...
    def __init__(self, window, device, BPF_fc1, BPF_fc2, LPF_fc, grid_type = 'none'):
        ...
        # Make BANDPASS filter variables based on user input
        if not BPF_fc1:
            # Set 10 Hz cut off frequency as default
            self.BPF_fc1 = 10
        else:
            self.BPF_fc1 = BPF_fc1
        
        if not BPF_fc2:
            # Set 500 Hz cut off frequency as default
            self.BPF_fc2 = 500
        else:
            self.BPF_fc2 = BPF_fc2

        self.sos_BPF = signal.butter(2, [self.BPF_fc1, self.BPF_fc2], 'bandpass', fs=self.sample_rate, output='sos')
        z_sos_BPF0 = signal.sosfilt_zi(self.sos_BPF)    # Lag variable
        self.z_sos_BPF = np.repeat(z_sos_BPF0[:, np.newaxis, :], self.num_channels-1, axis=1)
        
        # Make LOWPASSFILTER variables
        if not LPF_fc:
            # Set 10 Hz cut off frequency as default
            self.LPF_fc = 10
        else:
        ...
        self.sos_LPF = signal.butter(2, self.LPF_fc, 'lowpass', fs=self.sample_rate, output='sos')
        z_sos_LPF0 = signal.sosfilt_zi(self.sos_LPF)    # Lag variable
        self.z_sos_LPF = np.repeat(z_sos_LPF0[:, np.newaxis, :], self.num_channels-1, axis=1)
        
        self.envelope_view = True
    ...
    def setupThread(self):
        ...
        self.worker = EMGSamplingThread(self)
    ...
    def enableEnvelope(self):
        """ Enable the envelope for all channels
        """ 
        
        # Start envelope view
        self.worker.envelope_view = True
       
    def disableEnvelope(self):
        """ Disable the envelope for all channels 
        """
        # End envelope view
        self.worker.envelope_view = False

class EMGSamplingThread(QtCore.QObject):
    ...
    def __init__(self, main_class):
        ...
        self.envelope_view = main_class.envelope_view
        ...
        self.sos_BPF = main_class.sos_BPF
        self.z_sos_BPF = main_class.z_sos_BPF
        self.sos_LPF = main_class.sos_LPF
        self.z_sos_LPF = main_class.z_sos_LPF
    ...
    def update_samples(self): 
        ...
        while self.sampling:
            while not self.q_sample_sets.empty():
                ...
                # Only do the processing steps if you want the envelope view
                if self.envelope_view == True:
                    # Process all channels except STATUS and COUNTER channel
                    # 1. Filter samples -> BandPassFilter
                    filt_samples_BPF, self.z_sos_BPF = signal.sosfilt(self.sos_BPF, samples[:-2,:], zi = self.z_sos_BPF)
                    
                    # 2. Rectify samples
                    samples_rectified = np.abs(filt_samples_BPF)
    
                    # 3. Second time filtering -> Low Pass Filter
                    samples_envelope, self.z_sos_LPF = signal.sosfilt(self.sos_LPF, samples_rectified, zi = self.z_sos_LPF)
                    samples[:-2,:] = samples_envelope
